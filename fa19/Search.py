...
def a_star_search(grid, start, goal):
    ...
    assert 0 <= goal.x < len(grid[0]) and 0 <= end.y < len(grid), 'invalid stop point'
    heap = Heap(True)
...
def main():
    ...
    a.a_star_search(a.matrix[0][0], a.matrix[4][4])
    path = a.get_path(a[4][4])

    '''
    for i in range(a.rows):
        for j in range(a.columns):
            print(a.matrix[i][j].x, end = '')
        print('.')
    '''
